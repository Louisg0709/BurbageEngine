...
void Container::CreateEntity()
{
	if (std::is_base_of<Entity, EType>::value) {
		...
		if (MinID) {
			...
			//Send warning to the Containers log
		}
	}
	else {
		//Send warning to Containers log
	}
}

template<class EType>
std::vector<EType*> Container::GetEntitiesFromType()
{
...
void Container::RemoveEntity(ENTITY_ID_DATA_TYPE ID)
{
	ENTITY_ID_DATA_TYPE index = GetIndexFromId(ID);
	EntityArr.erase(EntityArr.begin() + index);
}

Entity* Container::GetEntityFromID(ENTITY_ID_DATA_TYPE ID)
{
	for (ENTITY_ID_DATA_TYPE i = 0; i < EntityArr.size(); i++) {
...
ENTITY_ID_DATA_TYPE Container::GetIndexFromId(ENTITY_ID_DATA_TYPE ID)
{
	for (ENTITY_ID_DATA_TYPE i = 0; i < EntityArr.size(); i++) {
		if (EntityArr[i].GetID() == ID) {
			return i;
...
ENTITY_ID_DATA_TYPE Container::FindMinAvailabeID()
{
...
*/
Entity::Entity(ENTITY_ID_DATA_TYPE id, Container* owner)
{
	EntityID = id;
	Owner = owner;
}
